...
public class RecognitionHelper {
	 ...
	 *         "ob":opponent black
	 */
/**	这段代码实现了从一个BufferedImage对象中获取棋盘上的一个棋子的颜色和所在位置的功能。
	具体实现步骤如下：
			1. 通过调用getStartXY方法获取棋盘的左上角的坐标（x,y）。
			2. 根据棋盘左上角的坐标（x,y），计算出上下两个棋子所在区域的左上角坐标（startUX,0）和（startLX,0）。
			3. 通过调用getSubimage方法获取上下两个棋子所在区域的BufferedImage对象。
			4. 调用getColor方法分别获取上下两个棋子的颜色。
			5. 根据获取到的颜色信息，判断上下两个棋子的颜色，从而确定棋子的颜色和所在位置。*/
